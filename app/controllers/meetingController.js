      ...
      UserModel.findOne({ userId: req.params.userId })
        ...
        .exec((err, userDetails) => {
          if (err) {
            ...
            logger.error(
              err.message,
              "Meeting Controller: findUserDetails",
              10
            );
        ...
        MeetingModel.find({ hostId: req.params.userId })
          ...
          .exec((err, meetingDetails) => {
            if (err) {
              ...
              logger.info(
                "No Meeting Found",
                "Meeting  Controller:findMeetings"
              );
              let apiResponse = response.generate(
                true,
                "No Meeting Found",
  ...
  MeetingModel.findOne({ meetingId: req.params.meetingId })
    ...
    .exec((err, meetingDetails) => {
      if (err) {
        ...
        logger.info(
          "No Meeting Found",
          "Meeting  Controller:getMeetingDetailsFunction"
        );
        let apiResponse = response.generate(
          true,
          "No Meeting Found",
...
let deleteMeetingFunction = (req, res) => {
      ...
      MeetingModel.findOne({ meetingId: req.params.meetingId })
        ...
        .exec((err, meetingDetails) => {
          if (err) {
            ...
            logger.error(
              err.message,
              "Meeting Controller: findMeetingDetails",
              10
            );
            let apiResponse = response.generate(
              true,
              "Failed To Find Meeting Details",
              500,
              null
            );
            reject(apiResponse);
  ...
  let deleteMeeting = (meetingDetails) => {
    return new Promise((resolve, reject) => {
      MeetingModel.findOneAndRemove({ meetingId: req.params.meetingId }).exec(
        (err, result) => {
          if (err) {
            console.log(err);
            ...
            logger.info(
              "No Meeting Found",
              "Meeting Controller: deleteMeeting"
            );
            let apiResponse = response.generate(
              true,
              "No Meeting Found",
            ...
            let apiResponse = response.generate(
              false,
              "Deleted the Meeting successfully",
              200,
              result
            );
            resolve(result);
...
let updateMeetingFunction = (req, res) => {
  ...
  let updateMeeting = (meetingDetails) => {
    return new Promise((resolve, reject) => {
      ...
      MeetingModel.update({ meetingId: req.params.meetingId }, options).exec(
        (err, result) => {
          if (err) {
            console.log(err);
  ...
  let validateUserInput = () => {
    return new Promise((resolve, reject) => {
      if (
        ...
        req.body.meetingPlace
      ) {
        resolve(req);
      } else {
        logger.error(
          "Field Missing During Meeting Creation",
        ...
        if (err) {
          ...
          let sendEmailOptions = {
            ...
            html: `<h3> Your meeting is Scheduled </h3>
                              <br> Hi , <br> ${newMeetingObj.hostName} has scheduled a meeting via Meeting Planner.
                              <br>  
          ...
          .exec((err, meetingDetails) => {
            if (err) {
              ...
              for (let meeting of meetingDetails) {
                if (time.isSameDayAsToday(meeting.meetingStartDate)) {
                  let sendEmailOptions = {
                    ...
                    html: `
                                              <p>This is a reminder about your upcoming meeting Today <br>
              ...
              if (i > 0) {
                let apiResponse = response.generate(
                  false,
                  "Meetings Found and sent reminders",
                  200,
                  null
                );

...
const applicationUrl = "http://meeting-planner.online/user"; //url of frontend application
...
/* Edit user details */
/* params : userId
   body : firstName,lastName,telephone 
*/
          ...
          if (err) {
            ...
            newUser.save((err, newUser) => {
              if (err) {
                ...
                console.log(
                  `${applicationUrl}/verify-email/${newUserObj.userId}`
                );
                ...
                let sendEmailOptions = {
                  ...
                  html: `<b> Dear ${newUserObj.firstName}</b> 
                                    <br>Welcome to our Meeting Planner App <br>
                                    Please <a href="${applicationUrl}/verify-email/${newUserObj.userId}">Click Here</a>  to verify your account.<br>
            ...
            logger.error("Already Exist", "userController: createUser", 4);
            let apiResponse = response.generate(true, "Email Exist", 403, null);
            reject(apiResponse);
          }
        }
      );
  ...
  let findUser = () => {
    ...
    return new Promise((resolve, reject) => {
      if (req.body.email) {
        ...
        //console.log(req.body);
        UserModel.findOne(
          { $and: [{ email: req.body.email }, { emailVerified: "Yes" }] },
          (err, userDetails) => {
...
let resetPasswordFunction = (req, res) => {
  ...
  let resetPassword = (tokenDetails) => {
    return new Promise((resolve, reject) => {
      ...
      UserModel.update({ email: req.body.email }, options).exec(
        (err, result) => {
          if (err) {
            console.log(err);
            logger.error(
              err.message,
              "User Controller:resetPasswordFunction",
              10
            );
            let apiResponse = response.generate(
              true,
...
let updatePasswordFunction = (req, res) => {
  let findUser = () => {
    ...
    return new Promise((resolve, reject) => {
      if (req.body.validationToken) {
        ...
        let apiResponse = response.generate(
          true,
          '"validationToken" parameter is missing',
          400,
          null
        );
        reject(apiResponse);
  ...
  let passwordUpdate = (userDetails) => {
    return new Promise((resolve, reject) => {
      ...
      UserModel.update({ userId: userDetails.userId }, options).exec(
        (err, result) => {
          if (err) {
            console.log(err);
            logger.error(
              err.message,
              "User Controller:updatePasswordFunction",
              10
            );
            let apiResponse = response.generate(
              true,
      ...
      passwordLib.comparePassword(
        ...
        (err, isMatch) => {
          if (err) {
            ...
            logger.info(
              "Update Failed Due To Invalid Password",
              "userController: validatePassword()",
              10
            );
            let apiResponse = response.generate(
